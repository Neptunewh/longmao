...
import type MorningSignRequest from '../types/requestTypes/MorningSignRequest';
import type MorningScoreRequest from '../types/requestTypes/MorningScoreRequest';
import type MorningTaskResponse from '../types/responseTypes/MorningTaskResponse';
import type MorningSignResponse from '../types/responseTypes/MorningSignResponse';
import type MorningScoreResponse from '../types/responseTypes/MorningScoreResponse';
...
const TotoroApiWrapper = {
  ...
  async getMorningSignTask(breq: BasicRequest): Promise<MorningTaskResponse> {
    return this.errorHandler.handleApiCall(async () => {
      const req: MorningTaskRequest = {
        ...breq,
        baseStation: '',
  ...
  async submitMorningSign(data: {
    ...
    pointId?: string
    breq: BasicRequest
  }): Promise<MorningSignResponse> {
    return this.errorHandler.handleApiCall(async () => {
      const req: MorningSignRequest = {
        ...data.breq,
  ...
  async getMorningSignScore(breq: BasicRequest): Promise<MorningScoreResponse> {
    return this.errorHandler.handleApiCall(async () => {
      const req: MorningScoreRequest = {
        ...breq,
        baseStation: '',
        mac: '02:00:00:00:00:00',
        phoneInfo: 'HUAWEI-P40|Android 12|EMUI 12.0',
      ...
      try {
        const response = await this.client
          .post('mornsign/getMornSignArchDetail', {
            body: encryptRequestContent(req),
          })
          .json<MorningScoreResponse>();
