...
/**
 * End-to-End Integration Tests for Free Run Feature
 * Tests complete user workflows from start to finish
 * Requirements: All requirements (comprehensive testing)
 */
describe('Free Run E2E Integration Tests', () => {
    ...
    describe('Complete Single Run Workflow', () => {
        it('should complete full single run workflow', async () => {
            ...
            const runData = await dataGenerator.generateRunData(templateParams, session.stuNumber, undefined, undefined, false)
    ...
    describe('Error Recovery Workflow', () => {
        it('should handle and recover from validation errors', async () => {
            ...
            const runData = await dataGenerator.generateRunData(correctedParams, session.stuNumber, undefined, undefined, false)
            expect(runData.distance).toBe('5.00')
            expect(runData.avgSpeed).toBe('10.00')
        })
    ...
    describe('Data Integrity Workflow', () => {
        it('should maintain data integrity throughout complete workflow', async () => {
            ...
            generatedRecords.forEach((record, index) => {
                expect(parseFloat(record.distance)).toBeCloseTo(testRuns[index].distance, 0.2)
                expect(parseFloat(record.avgSpeed)).toBeCloseTo(testRuns[index].avgSpeed, 0.2)
                expect(record.status).toBe('completed')
                expect(record.runType).toBe('1')
            })
            ...
            expect(parseFloat(sortedRecords[0].distance)).toBeCloseTo(3, 0.2)
            expect(parseFloat(sortedRecords[2].distance)).toBeCloseTo(8, 0.2)
